...
class LengthAwarePaginator extends AbstractPaginator implements Arrayable, ArrayAccess, Countable, IteratorAggregate, Jsonable, JsonSerializable, LengthAwarePaginatorContract
{
     ...
     * @return void
     */
    public function __construct(Request $request, $items, $total, $perPage, $currentPage = null, array $options = [])
    {
        $url = preg_replace('/[\?|\&]page=\d+/m', "", $request->fullUrl());
        ...
        $this->path = $url;//$this->path !== '/' ? rtrim($this->path, '/') : $this->path;
